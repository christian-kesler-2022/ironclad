        ...
        async function completeRegistration(valid) {
            if (valid) {
                connection.query(
                    ...
                    [email, username, hash, salt, username, '/images/contacts.png', '/styles/themes/light.css'],
                    function (error, results, fields) {
                        if (error) throw error;
                        function setVars() {
                    ...
                    function (error, results, fields) {
                        ...
                        if (results.length == 1) {
                            res.render('auth/reset-password', {
                                ...
                                email: email,
                                reset_key: reset_key,
                                green_message: "Hello " + username + ", please enter your new password"
                            });
                        } else {
                            res.render('auth/error', { red_message: "That link has expired" });
                        }
                    }
                )

            } catch (err) {
                res.render('auth/error', { red_message: "Something went wrong, please try again later" });
            }

        } else {
            res.render('auth/error', { red_message: 'You must enter valid parameters' });
        }
                    ...
                    function (error, results, fields) {
                        ...
                        if (results.changedRows == 1) {
                            res.render('auth/success', {
                                ...
                                pfp: req.session.pfp,
                                theme: req.session.theme,
                                green_message: 'Password changed, please Login <a href="/login">here</a>'
                            });
                        } else {
